...
class RegionRepositoryImpl @Inject constructor(
    ...
    override fun getAllProvince(): Flow<CommonState<List<Region>>> = flow{
        emit(CommonState.Loading())
        try {
            val mapper = RegionMapper()
            val response = regionRemoteDataSources.getProvinces()
            emit(CommonState.Success<List<Region>>(
    ...
    override fun getAllCity(idProvince: String): Flow<CommonState<List<Region>>> = flow{
        emit(CommonState.Loading())
        try {
            val mapper = RegionMapper()
    ...
    override fun getAllDistrict(idCity: String): Flow<CommonState<List<Region>>> = flow{
        emit(CommonState.Loading())
        try {
            val mapper = RegionMapper()
    ...
    override fun getAllVillage(idDistrict: String): Flow<CommonState<List<Region>>> = flow{
        emit(CommonState.Loading())
        try {
            val mapper = RegionMapper()
